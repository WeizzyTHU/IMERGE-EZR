...
# def update_firm(firm, firm_index, t, queue):
...
#               ub=[1, 1, 1, 1, 1], max_iter=100, precision=1e-3)
#     result = ga.run()
...
#     firm.Gamma2 = y2
#
#     logging.info(f'更新企业{firm.Firm_id}属性')
#     firm.update(t)
#
#     queue.put((firm_index, firm))
...
def main(parcel_excel, firm_excel):
    ...
    for t in range(1, T + 1):
        ...
        for firm in firms:
            ...
            # 率定
            ga = MyGA(func=calibrate_inner, n_dim=5, lb=[0, 0, 0, 0, 0],
                      ub=[1, 1, 1, 1, 1], size_pop=50, prob_mut=0.001, max_iter=1000)
            result = ga.run()
            a1, a2, a3, y1, y2 = result[0]
            firm.Utility = result[1]
            firm.Alpha1 = a1
        ...
        #     firm.global_vars = {}
        #     p = Process(target=update_firm, args=(firm, i, t, firm_queue))
        #     p.start()
        #     running_ps.append(p)
        #
        #     if len(running_ps) >= calibrate_process_num:
        #         while 1:
        #             try:
        #                 p = running_ps.pop()
        #                 p.join()
        ...
        #
        # while 1:
        #     if firm_queue.qsize() == len(firms):
        #         while not firm_queue.empty():
        #             firm_index, firm = firm_queue.get()
        #             firms[firm_index] = firm
        #         break
